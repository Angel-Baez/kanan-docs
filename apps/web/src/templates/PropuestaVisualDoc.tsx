    ...
    <div className="doc on">
      ...
      <div style={{ margin: "32px 0 18px" }}>
        ...
        >
          <EditableField
            ...
            size={32}
            placeholder="Tagline de la propuesta"
          />
        </div>
      ...
      <div className="ot3">
        {f.approaches.map((a, i) => (
          <div key={i} className="pv-ap">
            <span className="pv-ac">
              {a.code} · {" "}
              <EditableField
                value={a.title}
                onChange={(v) => {
                  const arr = f.approaches.map((x, j) =>
                    j === i ? { ...x, title: v } : x,
                  );
                  set("approaches", arr);
                }}
      ...
      <button
        ...
        onClick={() =>
          set("approaches", [
            ...f.approaches,
            {
              code: String.fromCharCode(65 + f.approaches.length),
              title: "",
              body: "",
            },
          ])
      ...
      <table>
        ...
        <tbody>
          {f.phases.map((p, i) => (
            <tr key={i}>
              <td className="sm">{p.number}</td>
              <td>
                <EditableField
                  value={p.phase}
                  onChange={(v) => {
                    const arr = f.phases.map((x, j) =>
                      j === i ? { ...x, phase: v } : x,
                    );
                    set("phases", arr);
                  }}
              ...
              <td className="r">
                <EditableField
                  value={String(p.investment)}
                  onChange={(v) => {
                    const arr = f.phases.map((x, j) =>
                      j === i ? { ...x, investment: parseFloat(v) || 0 } : x,
                    );
                    set("phases", arr);
                  }}
                  numeric
              ...
              <td>
                <span className="row-actions">
                  <button
                    className="row-del"
                    onClick={() => {
                      const filtered = f.phases.filter((_, j) => j !== i);
                      const renumbered = filtered.map((p, idx) => ({
                        ...p,
                        number: String(idx + 1).padStart(2, "0"),
                      }));
                      set("phases", renumbered);
                    }}
                  >
                    ×
                  </button>
      ...
      <button
        ...
        onClick={() =>
          set("phases", [
            ...f.phases,
            {
              number: String(f.phases.length + 1).padStart(2, "0"),
              phase: "",
              keyWorks: "",
              weeks: "",
              investment: 0,
      ...
      <div className="tot">
        <div className="ti">
          ...
          <div className="tv">RD$ {fmt(f.totalInvestment)}</div>
        </div>
        <div className="ti">
          <div className="tl">ITBIS (18%)</div>
          <div className="tv">RD$ {fmt(f.totalInvestment * 0.18)}</div>
        </div>
        <div className="ti g">
          <div className="tl">INVERSIÓN TOTAL</div>
          <div className="tv">RD$ {fmt(f.totalInvestment * 1.18)}</div>
        </div>
      </div>
      ...
      <div className="pv-trm">
        <div className="pv-trm-title">
          <EditableField
            value={f.ctaTagline}
            onChange={(v) => set("ctaTagline", v)}
            size={14}
        ...
        <div className="pv-trm-body">
          <EditableField
            ...
            size={30}
            placeholder="Frase de llamado a la acción"
            multiline
          />
        </div>
      </div>
      <div className="ot3">
        ...
        <div>
          ...
          <div className="cn">
            <EditableField
              value={f.reserveCondition}
              onChange={(v) => set("reserveCondition", v)}
              size={16}
            />
          </div>
        </div>
      ...
      <SignatureBlock
        signers={[
          { label: "KANAN Remodelaciones" },
          { label: f.clientName || "Cliente" },
        ]}
      />
      <DocFooter />
    </div>
